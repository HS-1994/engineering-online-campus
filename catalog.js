// Full Engineering Catalog layer: sourced personal library + university spine + elective subject bank.
(function(){
  ...
  // catalog-bootstrap.js already merges the elective bank into COURSE_LIBRARY.
  // Deduplicate here so the UI never shows the same elective twice.
  const seen = new Set();
  const all = core.filter(c=>{const k=String(c.courseId||c.id||c.title); if(seen.has(k)) return false; seen.add(k); return true;});
  ...
  const sourceLabel = c => c.sourceType || c.access || c.platform || 'Course Library';
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='catalog-css';s.textContent=`
      ...
      .catalog-card{border:1px solid rgba(196,220,240,.12);border-radius:16px;background:linear-gradient(155deg,rgba(18,34,52,.88),rgba(7,16,27,.88));padding:17px;display:flex;flex-direction:column;gap:10px;min-height:205px;transition:.18s ease}.catalog-card:hover{transform:translateY(-2px);border-color:rgba(99,216,255,.3)}
  ...
  function render(){
    ...
    return `<div class="catalog-backdrop" id="catalog-backdrop"><div class="catalog-shell"><header class="catalog-head"><div><div class="eyebrow">ENGINEERING ONLINE CAMPUS · MASTER CATALOG</div><div class="catalog-title">${total} learning options.</div><div class="catalog-sub">Core subjects stay in the degree spine; the master catalog adds the elective bank and your existing course library. Every catalog item exposes its study-access route and local progress.</div></div><button class="catalog-close" id="catalog-close">×</button></header><div class="catalog-tools"><input id="catalog-search" value="${esc(state.query)}" placeholder="Search courses, fields, topics…"/><select id="catalog-track">${tracks.map(x=>`<option ${x===state.track?'selected':''}>${esc(x)}</option>`).join('')}</select><select id="catalog-type">${types.map(x=>`<option ${x===state.type?'selected':''}>${esc(x)}</option>`).join('')}</select></div><div class="catalog-stat"><span>Showing <strong>${list.length}</strong></span><span>·</span><span>Available options <strong>${all.length}</strong></span><span>·</span><span>Active <strong>${active}</strong></span><span>·</span><span>Complete <strong>${completed}</strong></span></div><div class="catalog-grid">${list.map(card).join('')||'<div class="catalog-empty">No courses match those filters.</div>'}</div></div></div>`;
  }
  ...
  function bind(){
    ...
    document.getElementById('catalog-search')?.addEventListener('input',e=>{state.query=e.target.value;refresh()});
